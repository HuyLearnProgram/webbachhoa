...
- [x] **Phase 3** — LinUCB bandit + conversion attribution + A/B testing + dashboard đo lường. **Đã code xong + verify E2E qua HTTP thật (FE-style→Java→MySQL→Python), đủ drill: explore on/off cohort, 4 nhánh reward state machine, attribution qua checkout thật, auth 3 case, vòng khép kín click→poll→arm update.** Chạy trên dữ liệu seed gate (`../recommendation-service/scripts/seed_phase3_gate.py` + manifest `seed_manifest_phase3_20260710_210008.json` — bù đủ 8 tuần × ~2600-2800 AI-sourced impression/tuần + 230 conversion neo vào đơn PAID thật; xoá bằng `cleanup_seed_data.py <manifest>`, **nên xoá TRƯỚC khi tin số liệu bandit thật** vì bandit học trên click/convert seed trong thời gian test).
  ...
  - **Trạng thái cuối phiên (2026-07-11) — CHƯA COMMIT**: toàn bộ thay đổi Phase 3 (Java/Python/FE + docx/README/CLAUDE.md) đang ở working tree, chưa `git add`/`git commit`. Hai service dev đang chạy nền từ phiên này để phục vụ verify: Python `uvicorn` (:8000, log `recommendation-service/uvicorn_phase3.log`) và Spring Boot (:8080, log `server/server/springboot_phase3.log`) — cả 2 log này và `recommendation-service/data/` (SQLite bandit state) đã thêm vào `.gitignore` của `recommendation-service` nhưng **chưa kiểm tra `server/.gitignore`** có bỏ qua `springboot_phase3.log` chưa. Phiên sau: review diff toàn bộ trước khi commit, dừng 2 tiến trình dev nếu không cần nữa.
  - **Bước tiếp theo cho phiên sau**:
    1. Review + commit code Phase 3 (chưa làm trong phiên này vì không được yêu cầu).
    2. Để dashboard "Gợi ý AI" chạy quan sát một thời gian trên dữ liệu seed để đánh giá khả năng dự đoán của bandit (đúng mục tiêu ban đầu của user) — theo dõi cột A/B CTR delta, entropy shown-vs-clicked có tăng dần không.
    3. Khi đã đủ tin tưởng: xoá seed Phase 3 bằng `cleanup_seed_data.py scripts/seed_manifest_phase3_20260710_210008.json` **TRƯỚC** khi đọc số liệu bandit như số liệu thật (seed Phase 2 cũ `seed_manifest_20260710_153112.json` vẫn còn đó — cân nhắc xoá cùng lúc nếu đã đủ dữ liệu PAID orders thật để CF không tụt dưới guard).
    4. Cân nhắc sửa test `TC06` đang fail trong `Checkout.test.jsx` (xem mục 14 ở "Nợ kỹ thuật nhỏ" — phát hiện trong phiên này, KHÔNG phải do thay đổi Phase 3 gây ra, đã xác minh bằng `git stash`).
